...
declare type SelectPartialResult<TFields, TNullability extends Record<string, JoinNullability>> = SplitNullability<TNullability> extends infer TNullability extends Record<string, JoinNullability> ? TNullability extends TNullability ? {
    [Key in keyof TFields as Key extends string ? Key : never]: TFields[Key] extends infer TField ? TField extends AnySQLiteTable ? SelectPartialResult<GetTableConfig<TField, 'columns'>, TNullability> : TField extends AnySQLiteColumn ? GetColumnConfig<TField, 'tableName'> extends infer TTableName extends keyof TNullability ? ApplyNullability<SelectResultField<TField>, TNullability[TTableName]> : never : TField extends SQL | SQLResponse ? SelectResultField<TField> : TField extends Record<string, any> ? SelectPartialResult<TField, TNullability> : SelectResultField<TField> : never;
} : never : never;
export declare type AnySQLiteSelect = SQLiteSelect<any, any, any, any, any, any>;
export declare type BuildAliasTable<TTable extends AnySQLiteTable, TAlias extends string> = GetTableConfig<TTable> extends infer TConfig extends TableConfig ? SQLiteTableWithColumns<UpdateTableConfig<TConfig, {
    name: TAlias;
    columns: Simplify<MapColumnsToTableAlias<TConfig['columns'], TAlias>>;
}>> : never;
export declare type MapColumnsToTableAlias<TColumns extends Record<string, AnySQLiteColumn>, TAlias extends string> = {
    [Key in keyof TColumns]: ChangeColumnTableName<TColumns[Key], TAlias>;
};
export declare type AppendToResult<TTableName extends AnySQLiteTable, TResult, TJoinedName extends string, TSelectedFields extends SQLiteSelectFields, TOldSelectMode extends SelectMode> = TOldSelectMode extends 'partial' ? TResult : TOldSelectMode extends 'single' ? Record<GetTableConfig<TTableName, 'name'>, TResult> & Record<TJoinedName, SelectResultFields<TSelectedFields>> : Simplify<TResult & Record<TJoinedName, SelectResultFields<TSelectedFields>>>;
...
export declare type JoinFn<TTable extends AnySQLiteTable, TRunResult, TResultType extends 'sync' | 'async', TSelectMode extends SelectMode, TJoinType extends JoinType, TResult, TJoinsNotNullable extends Record<string, JoinNullability> = Record<GetTableConfig<TTable, 'name'>, 'not-null'>> = <TJoinedTable extends AnySQLiteTable, TJoinedName extends GetTableConfig<TJoinedTable, 'name'> = GetTableConfig<TJoinedTable, 'name'>>(table: TJoinedTable, on: SQL) => SQLiteSelect<TTable, TResultType, TRunResult, AppendToResult<TTable, TResult, TJoinedName, GetTableConfig<TJoinedTable, 'columns'>, TSelectMode>, TSelectMode extends 'partial' ? TSelectMode : 'multiple', AppendToJoinsNotNull<TJoinsNotNullable, TJoinedName, TJoinType>>;
export {};
